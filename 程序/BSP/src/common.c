...
/*******************************************************************************
 Function:      //
 Description:   //CRC16_Modbus
 Input:         //
 Output:        //
 Return:      	//
 Others:        //
*******************************************************************************/
uint16_t CRC16_Modbus(uint8_t *buff, uint16_t length)
{
  uint16_t crc = 0xFFFF;//16位crc寄存器预置
  uint16_t i = 0, j = 0;//计数
  
  for (i = 0; i < length; i++)//循环计算每个数据
  {
    crc ^= *buff;//将数据存入crc寄存器
...
//  {
//    value = value<<1; //低位向左移
//    value = ((data>>i)&0x01)|value; //低位的值
//  }
//  return value;
//}
/*******************************************************************************
 Function:      //
 Description:   //获取离散偏移时间,分
 Input:         //
 Output:        //
 Return:      	//
 Others:        //
...
uint16_t GetOffsetTime(uint8_t *buff)
{
  ...
  randomValue = ASCLL_to_Int(buff[0]);
  randomValue = randomValue *10 + ASCLL_to_Int(buff[1]);
  randomValue = randomValue *10 + ASCLL_to_Int(buff[2]);
  randomValue = randomValue *10 + ASCLL_to_Int(buff[3]);
...
void GetDeviceId(uint8_t *buff)
{
  ...
  buff[0] = ASCLL_to_Int(BC95.IMEI[0]);
  for(i = 1;i < 8;i++)
  {
    buff[i] = ASCLL_to_Int(BC95.IMEI[i*2-1])*0x10 + ASCLL_to_Int(BC95.IMEI[i*2]);
  }
}